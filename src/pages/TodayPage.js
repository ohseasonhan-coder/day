...
export default function TodayPage({ onNavigate }) {
  ...
  const recordedPercent   = children.length > 0
    ? Math.round((recordedChildIds.size / children.length) * 100)
    : 0;
  ...
  return (
    <div style={{ padding: '20px 20px 0' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, opacity: 0.8, marginBottom: 5 }}>
          {dateLabel} · {greeting} 👋
        </div>
        ...
        <div style={{ fontSize: 13, opacity: 0.82, marginTop: 5, lineHeight: 1.6 }}>
          {cl
            ? `${cl.year}학년도 · ${cl.age}세반 · 아이 ${children.length}명`
            : '반을 설정하면 자동화 현황을 볼 수 있어요'}
        </div>
      ...
      {unrecordedChildren.length > 0 ? (
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
            {unrecordedChildren.map(child => {
              ...
              return (
                <button
                  key={child.id}
                  onClick={() => onNavigate('record', { childId: child.id })}
                  style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--cat-play)', lineHeight: 1.4 }}>
                오늘 기록이 모두 완료됐어요 🎉
              </div>
              <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>
                문서함에서 보육일지 초안을 만들 수 있어요.
              </div>
            </div>
          </div>
          <button
            ...
            style={{
              fontSize: 12, fontWeight: 800, color: 'var(--cat-play)',
              background: 'white', border: '1.5px solid var(--cat-play)',
        ...
        <Section title="오늘 기록한 내용" action="문서 만들기" onAction={() => onNavigate('docs')}>
          ...
          {todayRecords.length > 3 && (
            <div style={{ textAlign: 'center', fontSize: 12, color: 'var(--text-tertiary)', marginTop: 4 }}>
              외 {todayRecords.length - 3}건 더 있어요
            </div>
          )}
        </Section>
      )}
...
function RecordMiniCard({ record }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ width: 9, height: 9, borderRadius: '50%', background: cat.color, marginTop: 7, flexShrink: 0 }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          <span style={{ fontWeight: 900, fontSize: 13, color: cat.color }}>{record.childName}</span>
          <span style={{ fontSize: 11, fontWeight: 800, color: cat.color, background: cat.bg, padding: '2px 8px', borderRadius: 100 }}>
            {cat.label}
          </span>
        </div>
...
function QuickAction({ icon, label, desc, color, onClick }) {
  return (
    ...
    >
      <div style={{ color, background: `${color}18`, width: 40, height: 40, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {icon}
      </div>
      <div>

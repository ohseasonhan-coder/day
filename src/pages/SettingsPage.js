...
function FormEditor({ form, onSave, onCancel }) {
  ...
  // .docx 파일 업로드 → 항목 자동 추출 (HWP·DOC는 안내)
  const handleDocxUpload = async (e) => {
    ...
    try {
      ...
      setDocxMsg(err.message || '파일을 읽는 중 오류가 발생했어요.');
    } finally {
      setDocxBusy(false);
    }
  };
  ...
  return (
    <div>
      ...
      {setupTab === 'paste' && (
        <div>
          {/* 워드(.docx) 파일 바로 올리기 */}
          <input ref={docxInputRef} type="file" accept=".docx,.hwp,.doc,.hwpx" onChange={handleDocxUpload} style={{ display:'none' }} />
          <button

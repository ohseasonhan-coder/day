...
async function callClaude(systemPrompt, userPrompt, maxTokens = 1500) {
  ...
  if (!apiKey) throw new Error('API 키가 설정되지 않았습니다. 설정에서 API 키를 입력해주세요.');
...
/**
 * Main record processing: categorize + soften + generate documents
 */
...
/**
 * Generate daily care journal (보육일지) from multiple records
 */
...
/**
 * Generate consultation document for a child
 */

...
import { applyOutsideQuotes as applyOutsideQuotesPreservingSpeech } from './normalizationRules';
import { buildDocumentMeta as buildModularDocumentMeta, formatDocumentUseLabels, getDocumentUsesForCategory } from './documentMeta';
...
function applyOutsideQuotes(text, rules) {
  return applyOutsideQuotesPreservingSpeech(text, (part) => {
      let result = part;
...
function buildDocumentMeta({ rawText, normalizedText, category, devAreas, tags, recordType, classAge }) {
  ...
  const usableFor = getDocumentUsesForCategory(category, recordType);
  ...
  const modularMeta = buildModularDocumentMeta({
    parsedInput: { rawText, normalizedText },
    categories: [category].filter(Boolean),
    devAreas: devAreas || [],
    tags: tags || [],
    documentUses: legacyMeta.usableFor,
    recordType,
    classAge,
  });
  return { ...legacyMeta, modularMeta };
}
...
function makeDocumentReadyText(meta) {
  ...
  void uses;
  const usable = formatDocumentUseLabels(meta.usableFor, { limit: 6 });

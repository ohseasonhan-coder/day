...
const LEARNING_SIGNALS = [
  // 빈도 상위 신호는 표현 풀 2종(결정론적 선택) — 반 전체에 같은 문장이 반복되는 것을 완화.
  // (7B 증류 후보는 품질 미달로 폐기, 어감만 참고해 수작업 정제. 모든 변형은 audit 무경고 검증 대상)
  { key: 'persist', label: '재시도·끈기', re: /(다시|무너지|넘어지|끝까지|반복|재시도|여러 번|계속|포기하지)/, needPeer: false,
    make: (t, src) => pickBy(src, [
      `${t} 뜻대로 되지 않는 순간에도 시도를 이어 가며 스스로 방법을 찾아가는 끈기를 보였다.`,
      `${t} 잘 되지 않는 순간에도 포기하지 않고 시도를 이어 가며 자신만의 해결 방법을 찾아갔다.`,
    ]) },
  ...
  { key: 'express', label: '표현·발화', re: /("[^"]+"|말하|이야기|설명|물어|불렀|노래|표현)/, needPeer: false,
    make: (t, src) => pickBy(src, [
      `${t} 자신의 생각과 느낌을 말과 행동으로 표현하며 놀이를 이끌어 가는 힘을 키워 갔다.`,
      `${t} 마음속 생각을 말로 풀어내며 놀이에 자신의 의미를 담아 가는 모습을 보였다.`,
    ]) },
  ...
  { key: 'explore', label: '탐색', re: /(관찰|탐색|비교|살펴|살피|궁금|실험|발견|돋보기|씨앗|달팽이|나뭇잎)/, needPeer: false,
    make: (t, src) => pickBy(src, [
      `${t} 주변을 자세히 살피고 궁금한 점을 탐색하며 알아 가는 즐거움을 경험하였다.`,
      `${t} 궁금한 대상을 찬찬히 들여다보며 스스로 답을 찾아가는 탐구의 즐거움을 누렸다.`,
    ]) },
  ...
  { key: 'make', label: '만들기', re: /(그리|색칠|만들|점토|블록|쌓|접|악기|율동|춤|꾸미|모양)/, needPeer: false,
    make: (t, src) => pickBy(src, [
      `${t} 재료를 자기만의 방식으로 다루며 만들고 표현하는 과정을 즐겼다.`,
      `${t} 손끝으로 재료를 매만지며 떠올린 것을 형태로 만들어 가는 과정에 몰입하였다.`,
    ]) },

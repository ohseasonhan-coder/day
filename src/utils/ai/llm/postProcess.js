...
export function validateLLMOutput({ data = {}, factCard = {}, input = '', observation = '', childName = '' } = {}) {
  ...
  if (learning && learning.length < 10) reasons.push('too_short');
  // 3) 기계적 반복
  ...
  if (reasons.length) return { ok: false, learning, support, audit: null, reasons };
  ...
  if (audit.severity === 'major') {
    return { ok: false, learning, support, audit, reasons: audit.warnings.slice() };
  }
  return { ok: true, learning, support, audit, reasons: [] };
}
